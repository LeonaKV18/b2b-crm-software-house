...
DROP TABLE meeting_participants CASCADE CONSTRAINTS;
...
CREATE TABLE invoices (
    ...
    CONSTRAINT fk_invoices_proposal FOREIGN KEY (proposal_id) REFERENCES proposals(proposal_id)
);
...
CREATE TABLE milestones (
    milestone_id NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
    proposal_id NUMBER NOT NULL,
    task_id NUMBER,
    description CLOB,
    fee NUMBER,
    ...
    CONSTRAINT fk_milestones_proposal
        FOREIGN KEY (proposal_id)
        REFERENCES proposals(proposal_id),
...
COMMENT ON TABLE milestones IS 'Stores milestone information for proposals.';
COMMENT ON COLUMN milestones.milestone_id IS 'Unique identifier for the milestone.';
COMMENT ON COLUMN milestones.proposal_id IS 'Foreign key to the proposal associated with this milestone.';
COMMENT ON COLUMN milestones.task_id IS 'Foreign key to the task associated with this milestone, if applicable.';
COMMENT ON COLUMN milestones.description IS 'Description of the milestone.';
COMMENT ON COLUMN milestones.fee IS 'Fee associated with the milestone.';
COMMENT ON COLUMN milestones.due_date IS 'Due date for the milestone.';
COMMENT ON COLUMN milestones.status IS 'Current status of the milestone (pending, completed).';
